...
func (r *Repository) ListWithBilling(ctx context.Context, f ListFilterBilling) ([]Group, error) {
	args := []any{}
...
func (r *Repository) GetGroupByIDForUpdate(ctx context.Context, tx pgx.Tx, groupID string) (*Group, error) {
...
func (r *Repository) FindGroupByStripeInvoiceID(ctx context.Context, invoiceID string) (*Group, error) {
	const q = `SELECT ` + groupSelectCols + `
		  FROM registration_groups
...
func (r *Repository) ListOverdueInvoiced(ctx context.Context, now time.Time) ([]Group, error) {
	const q = `SELECT ` + groupSelectCols + `
		  FROM registration_groups
